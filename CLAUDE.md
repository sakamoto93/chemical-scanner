...
ユーザー A:
  MacBook A (FastAPI サーバー)
  ├─ テザリング提供
  └─ iPhone A (Safari → 化学スキャナー使用)

ユーザー B:
  MacBook B (FastAPI サーバー)
  ├─ テザリング提供
  └─ iPhone B (Safari → 化学スキャナー使用)

ユーザー C:
  MacBook C (FastAPI サーバー)
  ├─ テザリング提供
  └─ iPhone C (Safari → 化学スキャナー使用)
```
...
2. **クライアント分離問題を回避**
   - Wi-Fi ルーターの「クライアント分離」が有効なため、端末間通信がブロック
   - 8月26日の ping テストで確認済み（172.28.1.134 → 172.28.1.13 で 100% packet loss）
   - テザリングはクライアント分離が適用されない
...
4. **既に動作確認済み**
   - テザリング接続での複数 iPhone アクセスは正常に動作
   - iPhone リアカメラ対応も確認済み
   - HTTPS（ポート 8443）での通信も確認済み
...
openssl req -x509 -newkey rsa:4096 -nodes \
  -out cert.pem -keyout key.pem -days 365 \
  -subj "/C=JP/ST=Tokyo/L=Tokyo/O=Lab/CN=localhost"
...
```
1. iPhone でテザリング有効化（パソコンに接続）
2. iPhone Safari を開く
3. アドレスバーに入力: https://172.20.10.x:8443
   （x.x は各自のテザリング IP の最後の部分）
4. セキュリティ警告 → 「詳細」→「このサイトに進む」
5. カメラ許可 → 「許可」
6. 試薬瓶を読み取り開始
```
...
```
パターン A: 各自のパソコン + テザリング で独立運用
→ 推奨（最もシンプル、トラブル少ない）

パターン B: 1 つの MacBook から複数 iPhone がテザリング接続
→ MacBook のバッテリー消費が多い、テザリング速度制限の可能性

パターン C: Wi-Fi ルーター経由（将来）
→ ルーター設定変更が必要になったときの代替案
```
